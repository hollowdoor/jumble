...
function readJSON(){
    ...
    for(var i=0; i<list.length; i++){
        ...
        if(fs.existsSync(current)){            
            ...
            json = fs.readFileSync(current, {encoding:'utf8'});
    ...
    if(json === null && fs.existsSync(path.join(cwd, 'index.html'))){
        
        htmlString = fs.readFileSync(path.join(cwd, 'index.html'), {encoding: 'utf8'});
        ...
        $links.each(function(i, el){
            ...
            if(rel === 'manifest' && json !== null){
                json  = fs.readFileSync(path.join(cwd, rel), {encoding: 'utf8'});
    ...
    data.files.concat(icosaves.files);
    data.directories.concat(icosaves.directories);
...
function getIcons(icons){
    ...
    for(var i=0; i<list.length; i++){
        ...
        }else if(icosaves.files.indexOf(current[0]) === -1){
            icosaves.directories.push(current[0]);
        }
    }
