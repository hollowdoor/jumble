...
function Massify(names, options){
    ...
    */
    ...
    function genBundle(name){
        ...
        The source variable is absolute so it doesn't include
        the dot at the beginning. It's more important that the
        exposed parameter matches what's bundled.
        */
        ...
        if(external.length){
            ...
            if(external.indexOf(name) !== -1)
                b.require(source, {expose: name.replace(/^\./, '')});//source, {expose: name.replace(/^\./, '')});
        }
